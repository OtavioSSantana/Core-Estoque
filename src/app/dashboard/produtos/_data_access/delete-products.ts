...
export async function deleteProduct(id: string | number) {
  try {
    ...
    // Verifica se o produto tem estoque em alguma loja
    const estoqueProduto = await prisma.estoque_loja.findFirst({
      where: { produto_id: productId }
    });

    if (estoqueProduto) {
      return NextResponse.json(
        { message: "Não é possível deletar produto que possui estoque em lojas" },
        { status: 409 }
      );
    }
...
export async function canDeleteProduct(id: string | number) {
  try {
    ...
    if (estoqueProduto) {
      return { 
        canDelete: false, 
        reason: "Produto possui estoque em lojas" 
      };
    }
