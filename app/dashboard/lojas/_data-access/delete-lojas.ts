...
export async function deleteLoja(id: number) {
  try {
    ...
    // Verifica se há movimentações de estoque relacionadas a esta loja
    const movimentacoes = await prisma.entrada_estoque.findFirst({
      where: { loja_id: id }
    });

    if (movimentacoes) {
      return NextResponse.json(
        { message: "Não é possível deletar uma loja que possui histórico de movimentações" },
        { status: 400 }
      );
    }
